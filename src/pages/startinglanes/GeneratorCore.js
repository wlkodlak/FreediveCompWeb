class GeneratorCore {
  constructor(settings, allStartingLanes, athletes) {
    this.startingLanes = getLeafLanes(allStartingLanes, settings.selectedStartingLane);
  ...
  getLeafLanes(startingLanes, parentLaneId) {
    for (const lane of startingLanes) {
      ...
      if (lane.rawLane.SubLanes && lane.rawLane.SubLanes.length > 1) {
        const leafLanes = [];
        buildLeafLanes(leafLanes, lane.rawLane.SubLanes);
        return leafLanes;
      } else {
        return [parentLaneId];
      }
  ...
  buildLeafLanes(output, inputs) {
    for (const input of inputs) {
      if (input.SubLanes && input.SubLanes.length > 0) {
        buildLeafLanes(output, input.SubLanes);
      } else {
      ...
      for (const announcement of athlete.Announcements) {
        if (this.disciplines.includes(announcement.DisciplineId)) {
          this.startingList.push({
            ...
            duration: extractDuration(announcement.Performance),
            distance: extractDistance(announcement.Performance),
            depth: extractDepth(announcement.Performance),
            startingLaneId: null,
            officialTop: null
          });
        }
  ...
  extractDuration(performance) {
    ...
    for (const part of performance.Duration.split(":")) {
      duration = duration * 60 + parseInt(part);
    }
    return duration;
  }
